...
프롬프트를 고칠 때마다 캐시 키가 바뀌어 다시 호출되므로, 완주 1회 비용보다
한 자릿수 큽니다.
...
6. **스캔 페이지는 PDF의 회전 메타데이터를 믿을 수 없습니다.** 약관 스캔 3장은
   `/Rotate` 값이 실제 스캔 방향과 달랐고, 렌더러가 그 값을 적용하는 바람에
   모델에 뒤집히거나 누운 이미지가 전달되고 있었습니다. 저장된 회전값을 무시하고
   콘텐츠 스트림 그대로 렌더하도록 바꾸고(해상도도 dpi 150으로 상향) 세 장 모두
   눈으로 확인해 해결했습니다. 메타데이터를 신뢰하기 전에 렌더 결과를 실제로
   열어봐야 한다는 것이 이 건의 교훈입니다.
