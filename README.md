...
- 응답은 `outputs/llm_cache/`에 캐싱되어, 캐시가 있는 재실행은 API 호출 없이
  동일하게 재현됩니다. 캐시는 커밋하지 않으므로 새로 clone한 환경의 첫 실행은
  LLM을 실제로 호출하며, 그룹핑 같은 경계 판단은 실행마다 달라질 수 있습니다
  (실측은 [실패 3](#package_01--채점-결과) 참조). `reference_run/`은 제출 시점
  실행의 산출물입니다.
...
표준 구조 순서로 대체 정렬합니다. 복원된 순서의 첫 장과 끝 장이 그 문서의
시작·끝 페이지이며, `documents.json`에 `start_page` · `end_page`로 적습니다.
순서 근거가 없어 정렬하지 못한 instance는 어느 장이 첫 장인지 말할 수 없으므로
두 값을 `null`로 둡니다 — 임의의 페이지를 시작이라 부르지 않습니다.
...
| 순서 복원 | 4/5 instance | 마커 경로로 복원된 것은 TITLE 1건뿐. URLA 2건·INCOME은 구조 순서로도 정답과 일치했고, CREDIT은 **실패** |
...
**실패 3 — URLA 11장이 두 문서로 갈림**

URLA는 한 부인데 4장과 7장, 두 instance로 나뉘었습니다. 4장은 ULI·대출번호가
찍힌 페이지, 7장은 차주 이름만 있고 ID 앵커가 없는 페이지입니다. 모델은
**앵커가 있는 쪽과 없는 쪽을 다른 문서로 봤습니다.** 서식 종류로 갈린 것이
아니라 양쪽 모두에 신청서 본체와 부속 서식이 섞여 있습니다.

나눌 이유는 없었습니다. 11장 모두 차주 이름이 같고, 페이지 마커도 `of 11`
한 세트로 1번부터 11번까지 다 있습니다. 프롬프트가 "이름이 단일하고 페이지
표기가 단일 세트면 1개로 확정하라"고 지시하는 바로 그 조건입니다.

순서 복원도 같이 무너졌습니다. 마커 정렬은 묶음 장수와 마커 분모가 같을 때만
쓰는데, 11장이 4장·7장으로 갈리면 `of 11`이 어느 쪽과도 맞지 않아 구조
순서로 내려갑니다.

원인은 코드가 아니라 **LLM 판단의 실행별 변동**입니다. 분리가 난 실행과 나지
않은 실행 사이에 그룹핑 코드·프롬프트 차이가 없음을 git 이력으로 확인했고,
캐시를 비운 콜드 실행 **4회 중 2회에서 재현**됐습니다(나머지 2회는 11장이
한 instance로 묶여 마커 경로로 복원). 분류 accuracy는 4회 모두 0.974로 같아,
흔들린 것은 그룹핑뿐입니다.

프롬프트는 분리 근거를 "페이지 번호 중복, ULI 상이, 서명 타임스탬프 상이"로
한정해 뒀는데, 모델은 "ULI 있음 vs 없음"으로 갈랐습니다. *상이*가 아니라
*부재*를 분리 근거로 쓴 오적용입니다. 앵커가 없는 페이지는 근거가 다른 것이
아니라 그 신호가 찍히지 않았을 뿐입니다. **앵커 부재는 분리 근거가 아니라고
명시**하는 것이 해소 방향입니다.

`reference_run/`은 분리가 난 실행을 그대로 두었습니다. 유리한 실행을 골라
커밋하는 것은 이 문서가 지켜온 원칙과 맞지 않습니다.
...
`documents.json`(그룹핑·순서·`start_page`/`end_page`)에 있습니다.
...
| instance | 복원 경로 | 복원된 순서 | 시작 | 끝 | 미해결 |
|---|---|---|---|---|---|
| urla_1 | 페이지 마커 | 20 35 13 16 43 18 23 29 25 37 | 20 | 37 | — |
| title_1 | 구조 순서 | 3 17 30 1 42 9 33 15 22 | 3 | 22 | 7, 5 |
| title_2 | 구조 순서 | 11 26 40 | 11 | 40 | — |
| credit_1 | 구조 순서 | 6 8 14 21 28 34 38 41 44 2 19 24 | 6 | 24 | 4, 10, 32 |
| income_1 | — | 순서 근거 없음 | — | — | 12, 27, 31 |

`income_1`은 순서를 세울 근거가 없어 시작·끝을 비웠습니다. 미해결 페이지가
있는 instance의 시작·끝은 **정렬된 페이지들 기준**이며, 미해결 페이지가 앞뒤
어디에 붙을지는 미정입니다.
...
4. **그룹핑 결과가 마커와 모순되면 코드가 되돌릴 장치가 없습니다.** 마커
   분모(`of 11`)가 묶음 장수와 어긋나면 그룹핑이 틀렸을 가능성이 높은데,
   현재는 순서 복원이 조용히 구조 순서로 내려갈 뿐 그룹핑을 재검토하지
   않습니다. package_01의 URLA 분리(실패 3)가 정확히 이 경우입니다. 마커
   분모와 묶음 크기의 일치를 그룹핑 결과의 검산으로 쓰고, 어긋나면
   재판정하거나 경고로 남기는 것이 개선 방향입니다.

### 입력 처리

5. **스캔 페이지의 위아래(0° vs 180°)는 판별하지 못합니다.**
