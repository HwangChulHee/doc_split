...
URLA는 한 부인데 4장과 7장 두 instance로 분리됐습니다. 페이지 마커가
`of 11`로 온전한데도 묶음이 갈리면서, 마커 정렬 조건(묶음 장수와 마커
분모 일치)을 만족하지 못해 순서 복원까지 구조 순서로 떨어졌습니다.
그룹핑 실패가 순서 실패로 이어진 사례입니다.

원인은 코드 회귀가 아니라 **LLM 그룹핑 판단의 실행별 변동**입니다. 분리가
나타난 실행과 나타나지 않은 실행 사이에 그룹핑 코드·프롬프트 차이가 없음을
git 이력으로 확인했고, 캐시를 비운 콜드 실행을 반복해 관측한 결과 **4회 중
2회에서 분리가 재현**됐습니다(나머지 2회는 11장 1개 instance로 묶여 마커
경로로 완전 복원). 페이지 분류 accuracy는 4회 모두 0.974로 동일해, 흔들리는
지점은 그룹핑에 한정됩니다. `reference_run/`은 분리가 나타난 실행을 그대로
두었습니다 — 유리한 실행을 골라 커밋하면 이 문서가 주장하는 원칙과
모순되기 때문입니다.
