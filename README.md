...
만들 필요는 없고, 만들어 넣어도 재귀적으로 찾습니다. 파일명에 `shuffled` 가
포함되면 분류 대상 패키지로, 나머지는 정답 원본으로 인식합니다 (파일명 키워드
...
- 규칙이 확정하지 못한 페이지만 LLM(gpt-5.4-mini)으로 보냅니다. 5개 유형
  후보와 규칙이 찾아낸 신호를 함께 넣어 페이지당 1회 호출합니다. 두 유형이
  동시에 결정적 신호를 낸 경합 페이지에서는 경합한 유형만 후보로 줍니다 —
  코드가 임의 우선순위를 두지 않으려는 것입니다.
...
실행하면 진행 상황이 `[1/8]` ~ `[8/8]` 로 찍히는데, 그 8단계가 아래와 같습니다.
...
| 그룹핑 | 3개 instance (URLA 11장 · CREDIT 18장 · TITLE 8장). INCOME 1장은 어느 instance에도 배정하지 않았습니다 |
| 순서 복원 | URLA 11장·TITLE 8장 **완전 복원**(페이지 마커 경로). CREDIT은 마커가 없어 구조 순서 폴백으로 14장을 배열하고 4장을 미해결로 남겼는데, 배열한 순서도 원본과 일치하지 않습니다 |
...
문서 구성은 총 5개 instance입니다 — URLA 10장, CREDIT 15장, INCOME 3장,
그리고 TITLE 2개(10장·3장)입니다. TITLE 10장짜리는 사실 거의 동일한
Commitment **두 벌**이 한 instance에 뭉쳐 있는 것입니다. 페이지 마커 중복
(`Page 1 of 5`가 두 장)으로 두 벌임은 감지했지만, 어느 장이 어느 벌인지 가를
근거가 신호 카드에 없어 분리하지 못했습니다(아래 한계 4번). 3장짜리는 마커
분모가 4로 다른 별도 출력본이고, 두 instance를 `related_to` 로 연결해 무엇이
다른지 근거를 남겼습니다.

귀속을 정하지 않은 페이지도 2장 있습니다(TITLE 1장, INCOME 1장). 어느
instance에 붙일 근거가 신호 카드에 없어서 억지로 배정하지 않았습니다.
상세한 사유는 `results/package_02/summary.md` 에 적어두었습니다.

### 비용

캐시가 없는 상태에서 두 패키지를 한 번 완주하는 비용은 **$0.069**입니다
(호출 18회 — 새로 clone 한 저장소에서 실측). 재실행은 캐시 적중으로 0원입니다.

개발 전 기간에 이 캐시로 실제 지출한 누계는 **$0.743**입니다 (호출 301회).
프롬프트를 고칠 때마다 캐시 키가 바뀌어 전부 다시 호출되기 때문에, 완주 1회
비용의 열 배가 넘습니다.
...
6. **스캔 페이지의 회전을 메타데이터로 판단하지 않고 무조건 무시합니다.** 약관
   스캔 3장은 `/Rotate` 값이 실제 스캔 방향과 달랐고, 렌더러가 그 값을 적용하는
   바람에 모델에 뒤집히거나 누운 이미지가 전달되고 있었습니다. 저장된 회전값을
   무시하고 콘텐츠 스트림 그대로 렌더하도록 바꿔(해상도도 dpi 150으로 상향) 세 장
   모두 정방향임을 눈으로 확인했습니다. 다만 이 처리는 "`/Rotate` 는 항상 틀렸다"고
   가정하는 것이라, 회전값이 내용과 맞는 PDF에서는 반대로 어긋납니다. 렌더 결과에서
   글자 방향을 판별해 회전을 정하는 것이 제대로 된 해법입니다.
