...
URLA는 한 부인데 7장과 4장, 두 instance로 나뉘었습니다. 7장은 신청서 본체,
4장은 부속 서식(렌더 기입란·추가서식·연속 기재지)입니다. 서식 종류가 다른
것을 다른 문서로 본 셈입니다.
...
쓰는데, 11장이 7장·4장으로 갈리면 `of 11`이 어느 쪽과도 맞지 않아 구조
순서로 내려갑니다.
...
프롬프트는 "URLA는 통상 1부"와 "같은 이름의 재제출본 가능성을 배제하지 마라"를
함께 지시합니다. 서식이 섞인 이 패키지에서는 두 지시가 팽팽해 실행마다 이기는
쪽이 달라진 것으로 보입니다. 서식 종류는 분리 근거가 아니라고 명시하는 것이
해소 방향입니다.
