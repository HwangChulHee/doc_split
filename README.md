...
4. **스캔 페이지의 위아래(0° vs 180°)는 판별하지 못합니다.**
   회전은 PDF에 저장된 회전값이 아니라 렌더한 이미지에서 정합니다 — 행별
   잉크량의 요동으로 글줄이 흐르는 축을 재서 정방향으로 세우고, 두 축이 갈리지
   않으면 판정을 포기하고 방향별 점수와 함께 그 사실을 판정 기록에 남깁니다
   (`ingest/render.py`, 텍스트 페이지 78장에서 오판 0건·미판별 1건). 다만 이
   방법은 **축까지만** 가릅니다. 180° 뒤집힌 글줄은 바로 선 글줄과 잉크 분포가
   같기 때문입니다. 대신 쓸 수 있는 특징(글줄 안에서 잉크가 위아래 어디에
   몰리는지)을 실제 페이지 76장에 재 봤지만 65장만 맞았고 이 데이터셋의 스캔
   3장은 전부 틀려(대문자 위주 약관에는 디센더가 거의 없음) 채택하지
   않았습니다. 확실히 하려면 글자를 읽어야 하는데, OCR·OSD 엔진을 의존성으로
   들이는 것은 제출 환경 재현성과 맞바꾸는 선택이라 하지 않았습니다.
...
- **대량 처리**: 페이지 판정이 서로 독립이고 규칙 판정은 CPU bound라
  multiprocessing 병렬화가 성립합니다. 직렬/병렬 판정 동일성과 처리량은
  [`scripts/bench_parallel.py`](scripts/bench_parallel.py)로 실측할 수
  있습니다. 현재 실행 경로는 순차 처리입니다.
