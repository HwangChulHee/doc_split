...
규칙 판정은 정규식·문자열 정규화라 CPU bound이고, 페이지 간 의존이 없습니다
(정책은 읽기 전용, 공유 상태 없음). 그래서 스레드가 아니라 프로세스 단위로
그대로 갈라집니다. 합성 페이지 2000장 실측:

| 프로세스 | pages/s | speedup |
|---|---|---|
| 1 | 78 | 1.00x |
| 2 | 142 | 1.81x |
| 4 | 230 | 2.92x |
...
Intel i5-14500(6P+8E, 20스레드) 기준. `uv run python scripts/bench_parallel.py
--pages 2000 --scaling` 으로 재현할 수 있고, 스크립트가 매 실행 **병렬 판정이
직렬과 완전히 일치하는지**(유형·등급·subtype) 검증합니다. 8프로세스까지 거의
선형이고 그 이상은 E코어·하이퍼스레딩 구간이라 기울기가 꺾입니다.

측정에는 실제 문서 대신 정책의 공개 표준 문구(GSE 푸터·ALTA 고지문 등)와
무작위 채움말로 만든 합성 페이지를 씁니다. 결정적 신호가 있는 페이지와 신호가
전무한 페이지를 섞었습니다 — 신호 없는 페이지가 가장 비싸기 때문에(4개 정책을
전부 소진) 좋은 쪽만 재지 않도록 한 것입니다.

대량 처리에서 실제 병목은 규칙 단계가 아니라 LLM 단계입니다. 다만 그쪽은
네트워크 대기라 프로세스를 늘리는 문제가 아니라 동시 요청 수와 캐시 적중률의
문제이고, 규칙이 83페이지 중 73페이지를 확정하므로 LLM으로 넘어가는 비율
자체가 낮습니다.
