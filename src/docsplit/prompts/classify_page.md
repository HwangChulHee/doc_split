...
수정 이력:
  v1  최초 — DEFER_LLM / TYPE_CONFLICT 두 경로 공용
  v2  NO_SIGNAL 경로 추가, 기능 기준(발행 경로 ≠ 문서 기능) 지시 추가
  v3  기능 기준을 "무엇을 확인하는가"로 구체화하고 벤더 스탬프를 근거에서 제외
  v4  통합 판정 도입 후 자기신고형 소득 문서가 밀려나는 회귀 대응 —
      소득 증빙에 제3자 검증본만 있는 것이 아님을 하위군 일반 서술로 보강
-->
...
2. 판단 기준은 **이 페이지가 어떤 문서의 일부인가**(문서의 기능)이다.
   ...
   - **소득 증빙은 제3자 검증 문서만이 아니다.** 국세청 기록이나 고용 검증처럼
     제3자가 확인해 준 문서 외에, 차주 본인이나 세무사가 작성한 소득 정리
     (손익계산서, 소득 신고 요약 등)도 `INCOME_DOC`에 속한다.
     `Profit`·`Income` 같은 라벨이 없어도, **금액의 구조**(수입·지출·합계가
     나열된 형태)와 **작성자 맥락**(세무사 서명, 세무대리인 등록번호)으로
     문서의 기능을 추론하라.
