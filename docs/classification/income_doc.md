...
구현된 I-V6는 이 표보다 넓다: p26 하나가 아니라 **기대 7p 전부가 최종적으로
INCOME_DOC 으로 확정되는지**를 본다 (p26 자체는 해소됐다). 결과는 4/7 FAIL이고
남은 3건의 성격은 `known_limits.md` §6에 적었다 — 설계 시점의 좁은 기준을
구현이 넓혀 잡은 것이라 그대로 두었다.

I-V4도 기준대로 나오지 않았다. 최종 실행의 INCOME 그룹핑은 **transcript 3장이
한 instance**이고(두 벌이 갈리지 않았다) 검증 대행 1장은 미배정이다. 10자 토큰이
두 벌을 관통해 앵커가 되지 못한다는 §2-4의 관찰이 그대로 결과로 나타난 것이며,
가를 근거를 만들어내지 않고 실패로 두었다.
