...
**자기들끼리 한 instance(`title_2`)로 묶였다.** 렌더가 회전을 바로잡고 VLM이
...
```text
렌더링: PyMuPDF pixmap (dpi 150) — 렌더 결과에서 글줄 축을 판별해 정방향으로 렌더
...
마커·폼 코드·벤더명까지 판독됐다. 그 뒤 "무조건 무시"라는 전제도 지웠다: 이제
`ingest/render.py`가 렌더 이미지의 행별 잉크 요동으로 글줄 축을 판별해 회전을
정하고(3장 모두 "가로, 0°" — 같은 렌더이므로 위 판독 결과와 캐시가 그대로 유지),
판별이 갈리지 않으면 종전 동작으로 폴백한다. 위아래(0°/180°)는 여전히 판별하지
못하고 미판별로 기록한다 (`known_limits.md` §5).
